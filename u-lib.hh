...
inline int sys_page_free(void* addr) {
    return make_syscall(SYSCALL_PAGE_FREE, (uintptr_t) addr);
}

inline int sys_munmap(void* addr, size_t size) {
    return make_syscall(SYSCALL_MUNMAP, (uintptr_t) addr, size);
}

inline int sys_check_munmap(uintptr_t addr, size_t length) {
    return sys_check_munmap(addr, length);
}

inline int syscall_sleep(pid_t pid, size_t duration) {
    return make_syscall(SYSCALL_SLEEP, pid, duration);
}
